...
void Orchestrator::ConfigurationManager::loadConfiguration(
  ...
  try {
    ...
    throw ConfigurationParseException(std::string(pex.getFile()) + ":" +
...
std::string
Orchestrator::ConfigurationManager::getStringValue(const std::string key) {
  try {
    std::string value;
...
void Orchestrator::ConfigurationManager::modifyEntry(
