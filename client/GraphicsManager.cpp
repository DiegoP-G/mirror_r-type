...
bool GraphicsManager::init(const std::string &title, int width, int height) {
  ...
  if (!font.loadFromFile("../assets/fonts/upheavtt.ttf")) {
    std::cerr << "Warning: SFML Failed to load font, using default font"
              << std::endl;
...
void GraphicsManager::clear() { window.clear(sf::Color::Black); }

void GraphicsManager::present() { window.display(); }

sf::Texture &GraphicsManager::createColorTexture(int width, int height,
                                                 sf::Uint8 r, sf::Uint8 g,
