...
public:
  FlappyBirdGame()
      : rng(std::random_device{}()), pipeHeightDist(100.0f, 400.0f) {}
  ...
  void createTextures() {
    SDL_Texture *birdTexture =
        g_graphics->createColorTexture(32, 32, 255, 255, 0); // Yellow bird
    SDL_Texture *pipeTexture =
        g_graphics->createColorTexture(80, 400, 0, 255, 0); // Green pipes
  ...
  void createBird() {
    ...
    birdEntity.addComponent<JumpComponent>(-400.0f);
  ...
  void spawnPipe() {
    ...
    bottomPipe.addComponent<SpriteComponent>(
        80, static_cast<int>(600 - pipeHeight), 0, 255, 0); // Green
    ...
    topPipe.addComponent<SpriteComponent>(80, static_cast<int>(topPipeHeight),
                                          0, 255, 0); // Green
    ...
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_SPACE) {
        if (!gameStarted) {
          ...
          auto entities =
              entityManager.getEntitiesWithComponents<InputComponent>();
