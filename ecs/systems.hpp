...
public:
    void update(EntityManager& entityManager) {
        ...
        for (size_t i = 0; i < entities.size(); i++) {
            if (!entities[i]->isActive()) continue; // Skip inactive entities
            ...
            for (size_t j = i + 1; j < entities.size(); j++) {
                if (!entities[j]->isActive()) continue; // Skip inactive entities
...
private:
    void handleCollision(Entity* a, Entity* b) {
        ...
        bool aIsPlayer = !a->hasComponent<EnemyComponent>() && a->hasComponent<InputComponent>();
        bool bIsPlayer = !b->hasComponent<EnemyComponent>() && b->hasComponent<InputComponent>();
...
public:
    void update(EntityManager& entityManager, float deltaTime) {
        ...
        for (auto& entity : entities) {
            ...
            // Reset velocity
            velocity.velocity = Vector2D(0, 0);
            
            // Apply movement based on input
            if (input.up) velocity.velocity.y = -velocity.maxSpeed;
            if (input.down) velocity.velocity.y = velocity.maxSpeed;
            if (input.left) velocity.velocity.x = -velocity.maxSpeed;
            if (input.right) velocity.velocity.x = velocity.maxSpeed;
            
            // Handle firing
            if (input.fire && entity->hasComponent<TransformComponent>()) {
                fire(entityManager, entity);
                input.fire = false; // Reset firing state
            }
        }
    }
...
private:
    void fire(EntityManager& entityManager, Entity* player) {
        ...
        projectile.addComponent<TransformComponent>(
            transform.position.x + 20.0f, // Offset to fire from the front of the player
            transform.position.y + 10.0f  // Center height
        );
        
        // Add projectile components
        projectile.addComponent<VelocityComponent>(300.0f, 0.0f); // Fast horizontal movement
        projectile.addComponent<SpriteComponent>(); // Add sprite
        projectile.addComponent<ColliderComponent>(10.0f, 5.0f); // Small hitbox
        projectile.addComponent<ProjectileComponent>(10.0f, 2.0f, player->getID()); // Damage, lifetime, owner
    }
};
...
private:
    void enemyFire(EntityManager& entityManager, Entity* enemy) {
        ...
        projectile.addComponent<SpriteComponent>(); // Add sprite (different from player projectiles)
        projectile.addComponent<ColliderComponent>(10.0f, 5.0f); // Small hitbox
