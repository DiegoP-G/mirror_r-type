...
public:
  void update(EntityManager &entityManager) {
    ...
    for (auto &entity : entities) {
      ...
      if (sprite.texture) {
        g_graphics->drawTexture(*sprite.texture, static_cast<int>(position.x),
                                static_cast<int>(position.y), sprite.width,
                                sprite.height);
      } else {
...
private:
  void spawnLaser1(EntityManager &entityManager) {
    ...
    laser.addComponent<LaserWarningComponent>(
        width, height, 5.0f, 1.0f, 3.0f); // Warning for 2s, active for 3s
  }
