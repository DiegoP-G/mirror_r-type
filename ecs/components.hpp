...
// Vector2D pour les positions et mouvements
class Vector2D
{
  public:
    float x, y;
...
// Rectangle pour les collisions et le rendu
class Rectangle
{
  public:
    float x, y, w, h;
...
// Composant pour la position, l'échelle et la rotation
class TransformComponent : public Component
{
  public:
    ...
    TransformComponent(float x, float y, float sx, float sy, float r) : position(x, y), scale(sx, sy), rotation(r)
    {
    }
...
class PlayerComponent : public Component
{
  public:
    ...
    PlayerComponent(int playerID = 0) : score(0), lives(3), shootCooldown(0.0f), playerID(playerID)
    {
    }
...
// Composant pour l'avertissement laser
...
// Composant pour centrer les entités à l'écran
...
// Composant pour le rendu avec SFML
class SpriteComponent : public Component
{
  public:
    sf::Texture *texture; // Note: Ne peut pas sérialiser le pointeur de texture
    Rectangle srcRect;
    int width, height;
    bool isVisible;
    uint8_t r, g, b, a;

    SpriteComponent() : texture(nullptr), isVisible(true), width(32), height(32), r(255), g(255), b(255), a(255)
    {
    }
    ...
    // Sérialisation (excluant le pointeur de texture)
    std::vector<uint8_t> serialize() const
    {
        std::vector<uint8_t> data;
    ...
    static SpriteComponent deserialize(const uint8_t *data)
    {
        ...
        comp.b = data[offset++];
        comp.a = data[offset];
        comp.texture = nullptr; // La texture doit être recréée
        return comp;
    }
};

// Composant pour les collisions
...
// Composant pour la santé des joueurs/ennemis
...
// Composant pour les entrées du joueur
...
// Composant pour les projectiles
class ProjectileComponent : public Component
{
  public:
    ...
    ProjectileComponent(float d, float lt, EntityID o) : damage(d), lifeTime(lt), remainingLife(lt), owner(o)
    {
    }
...
// Composants spécifiques à Flappy Bird
...
// Composant de saut générique au lieu de BirdComponent
