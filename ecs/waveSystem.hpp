...
inline std::vector<Vector2D> diamondPattern(int count, float cx, float cy) {
    ...
    for (int i = 0; i <= layers && placed < count; ++i) {
        ...
        if (i == 0) {
            ...
            if (placed < count) positions.push_back(Vector2D(cx - i * spacing, y)); // Left
            placed++;
            if (placed < count) positions.push_back(Vector2D(cx + i * spacing, y)); // Right
            placed++;
        }
    }
    ...
    for (int i = layers - 1; i >= 0 && placed < count; --i) {
        ...
        if (i == 0) {
            ...
            if (placed < count) positions.push_back(Vector2D(cx - i * spacing, y - (120 * (i + 1)))); // Left
            placed++;
            if (placed < count) positions.push_back(Vector2D(cx + i * spacing, y - (120 * (i + 1)))); // Right
            placed++;
        }
    }
...
class WaveSystem
{
  ...
  public:
    void update(EntityManager &entityManager, float deltaTime)
    {
        ...
        if (waveActive &&
            entityManager.getEntitiesWithComponents<EnemyComponent>().empty())
        {
